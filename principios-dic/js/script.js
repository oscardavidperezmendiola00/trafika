!(function() {
  ...
  document.documentElement.classList.add("is-loaded"),
    ...
    setTimeout(function() {
      var n = [], l = [];
      var i = null;
      var elWrap = document.querySelector('#js-scroll');
      if (elWrap) {
        i = new z({
          el: elWrap,
          smooth: !0,
        ...
        i.on("call", function(t, e, data) {
          if ("dynamicBackground" === t) {
            if ("enter" === e) n.push({ id: data.id, el: data.el });
            else for (var s = 0; s < n.length; s++) if (data.id === n[s].id) { n.splice(s, 1); break; }
          } else if ("dynamicColor" === t) {
            if ("enter" === e) l.push({ id: data.id, el: data.el });
            else for (var s2 = 0; s2 < l.length; s2++) if (data.id === l[s2].id) { l.splice(s2, 1); break; }
          }
        });
      }
    }, 1e3);
